...
gpio_write_bit(PIN_PORT, PinIndexOnPortB, HIGH);
   ...
   // set high
   GPIOB->regs->BSRR = BIT(PinIndexOnPortB);
   // set low
   PINPORT->regs->BRR = BIT(pin);

//-----------------------------------------------------------
* Also LOOK INTO Colour problem -- see BLACK Tab in Arduino TFT code
* -----------------------------------------------------------
* Also modify code to allow user to set SPI channel and SPI speed 
* when creating an instance of the ST7735 class in a sketch
* -----------------------------------------------------------
*/

#include "Adafruit_ST7735.h"

#define SW_SPI_RATE	 10
...
#include <SPI.h>
#else
#pragma message("compiling for DUE ...")
#include <SPI.h>
#include "pins_arduino.h"
#endif
*/

//bobk - Maple SPI class
HardwareSPI spi(1);		//using channel 1 for now


// Constructor when using software SPI.  All output pins are configurable.
Adafruit_ST7735::Adafruit_ST7735(uint8_t cs, uint8_t rs, uint8_t sid,
 uint8_t sclk, uint8_t rst)
  : bk_GFX(ST7735_TFTWIDTH, ST7735_TFTHEIGHT) {
  _cs   = cs;
  _rs   = rs;
  _sid  = sid;
  _sclk = sclk;
...
inline void Adafruit_ST7735::spiwrite(uint8_t c) {
  ...
  //Serial.println(c, HEX);
	...
	if (hwSPI) {
		//uint8_t response = SPI.transfer(c);  ----  orig for Due
		uint8_t discard = spi.transfer(c);
		
		// transfer above should be better
		//spi.write(c);
	...
	*/
		// bobk: using slower digitalWrite()
		int bitcnt;		
		for (bitcnt=8; bitcnt>0; bitcnt--)
		{
			digitalWrite(_sclk, 0);                             // Set Clock Idle level LOW.
			if ((c & 0x80)==0x80) {digitalWrite(_sid, 1);}	// PCD8544 clocks in the MSb first.
			else {digitalWrite(_sid, 0);}
			digitalWrite(_sclk, 1);		// Data is clocked on the rising edge of SCK.
			c = c << 1;	// Logical shift data by 1 bit left.
			delayMicroseconds(SW_SPI_RATE);	// 10uS is 100kHz.  sysclk is 25nS
		}
    }
}
...
#define DELAY 0x80
static const uint8_t 			//prog_uchar
...
/* Bobk -- In Arduino.h portOutputRegister() is defined as:
#define portOutputRegister(port)   ( &(port->PIO_ODSR) )

And in Variants.h  BOARD_SPI_DEFAULT_SS is defined as:
...
void Adafruit_ST7735::commonInit(const uint8_t *cmdList) {
  ...
  //constructor(ST7735_TFTWIDTH, ST7735_TFTHEIGHT);   -- bobk: moved to ST7735 constructor
  colstart  = rowstart = 0; // May be overridden in init func

  pinMode(_rs, OUTPUT);
  pinMode(_cs, OUTPUT);
  // bobk: Think following is done to avoid digitalWrite()
  /*
  ...
  */
  if(hwSPI) { // Using hardware SPI
	...
	spi.begin(SPI_281_250KHZ, MSBFIRST, SPI_MODE_0);
